...
struct LinkedListSet* initialiseLLSet(){
    struct LinkedListSet* set = malloc(sizeof(struct LinkedListSet));
    memset(set->booleanArr, false, sizeof(set->booleanArr));

    set->linkedList = malloc(sizeof(struct LinkedListArray));
...
{
    struct Sets *newSets = malloc(sizeof(struct Sets));
