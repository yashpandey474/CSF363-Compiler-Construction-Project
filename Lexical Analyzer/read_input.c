...
int readIntoBuffer(BufferArray *bufferArray)
{
    char *buffer;
...
BufferArray *initialiseTwinBuffer(FILE *file)
{
    // INITIALISE BUFFER
    BufferArray *bufferArray = (BufferArray *)malloc(sizeof(BufferArray));
...
void equivalentNumber(twinBufferlex, int flag, struct SymbolTableEntry *token)
{
    // printf("EQV NUMS %s\n", token->lexeme);
...
twinBufferinitialiseLA(BufferArray *bufferArray)
{
    twinBuffer LA;
