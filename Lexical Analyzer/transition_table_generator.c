...
int reinitialiseArrays()
{
    ...
    for (int row = 0; row < NUM_STATES; row++)
    {
        ...
        int j = 0;
        while (j + INPUT_SECOND_DIMENSION < STATE_ARRAY_SIZE_TEMP_GEN)
    ...
    {
        if (checkStateTemp[actualSizeUsed] == -1 && nextStateTemp[actualSizeUsed] == -1)
        {
            actualSizeUsed -= 1;
        }
        else
    ...
    fclose(file);
    printArray("nextState", nextStateTemp, actualSizeUsed, "int", "STATE_ARRAY_SIZE");
    printArray("checkState", checkStateTemp, actualSizeUsed, "int", "STATE_ARRAY_SIZE");
